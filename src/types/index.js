export const SUCCESFULL_LOGIN = "SUCCESFULL_LOGIN";
export const ERROR_LOGIN = "ERROR_LOGIN";
export const GET_USER = "GET_USER";

export const SELECT_DEVICE = "SELECT_DEVICE";
export const SELECT_PLAN = "SELECT_PLAN";
export const SELECT_COVERAGE = "SELECT_COVERAGE";
export const SELECT_DOCUMENTS = "SELECT_DOCUMENTS";
export const ERROR_SELECT = "ERROR_SELECT";
export const SUBMIT_FORM = "SUBMIT_FORM";
export const ERROR_FORM = "ERROR_FORM";
export const CLEAR_FORM = "CLEAR_FORMA";
export const MORE_INFO = "MORE_INFO";
export const DEVICES_USER = "DEVICES_USER";
export const PLANS_DEVICE = "PLANS_DEVICE";
export const COVERAGE_PLAN = "COVERAGE_PLAN";
export const DOCS_COVERAGE = "DOCS_COVERAGE";

export const NEXT_STEP = "NEXT_STEP";
export const PREVIUS_STEP = "PREVIUS_STEP";